...
//la usiamo in generate_thread, in croc
struct croc_params {
    int flusso;
};

//la usiamo in generate_thread,in bullet_croc e in bullet
struct bull_params {
    int x;
    int y;
    int type;
...
extern struct msg buffer[DIM_BUFFER];
extern sem_t sem_liberi, sem_occupati;
extern pthread_t fcroc_tid;
#endif //DEFINIZIONI_H
